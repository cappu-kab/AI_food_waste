...
(function (global) {
  ...
  function decisionEngine({ contaminationTotal, solidWaste, freeLiquidL, cn, mmix, plantFraction }) {
    ...
    if (contamRatio > 0.02) {
      ...
      reasons.push(
        `สัดส่วนการปนเปื้อน ${(contamRatio * 100).toFixed(1)}% ของขยะอาหารแข็ง (> 2%)`
      );
      prep.push("คัดแยกพลาสติกและสิ่งปนเปื้อนก่อนนำไปหมัก");
      status = "warn";
    }

    if (freeLiquidL > 0) {
      if (recommendation === "COMPOST CANDIDATE") recommendation = "SEPARATE LIQUID";
      reasons.push(`มีของเหลวอิสระ ${freeLiquidL} L ต้องแยกออกจากส่วนผสมแข็ง`);
      prep.push("แยกน้ำซุป/น้ำแกงออกจากขยะอาหารแข็ง");
      status = "warn";
    }
    ...
    if (cnOut || mOut) {
      ...
      if (cnOut) reasons.push(`อัตราส่วน C:N = ${cn.toFixed(1)} อยู่นอกช่วง 20–40`);
      if (mOut) reasons.push(`ความชื้นผสม = ${moisturePct.toFixed(1)}% อยู่นอกช่วง 45–65%`);
      prep.push("ปรับด้วยวัสดุคาร์บอน/ไนโตรเจน หรือจัดการความชื้นก่อนหมัก");
      status = "adjust";
    }

    if (recommendation === "COMPOST CANDIDATE") {
      reasons.push("C:N และความชื้นอยู่ในช่วงที่หมักได้ และการปนเปื้อนอยู่ในเกณฑ์");
      prep.push("กองหมักตามมาตรฐาน กลับกอง และตรวจอุณหภูมิ");
    }

    let vermiNote = null;
    if (plantFraction >= 0.85) {
      vermiNote =
        "VERMICOMPOST CANDIDATE AFTER PRE-TREATMENT — สัดส่วนพืช ≥ 85% (ต้องผ่านการเตรียมก่อน)";
      prep.push("หากใช้ไส้เดือน ต้องเตรียมวัตถุดิบ (pre-treatment) ก่อน");
    }
